...
function updateTime() {
    ...
    document.getElementById("time").textContent = "Time: " + elapsedTime; // Update the displayed time
}
...
// Stop the timer when the game ends (optionwdal)
function stopTimer() {
    clearInterval(timerInterval); // Stop time
    let finalTime = Math.floor((Date.now() - startTime) / 1000);
    alert("Game Over! Your time was: " + finalTime + " seconds.");
}

// Example usage: Start timer when the game loads
startTimer();
class Game {
    constructor() {
        // Set up canvas
    ...
    createPlayer() {
        ...
        const playerBody = this.world.createBody({
            type: 'dynamic',
            position: planck.Vec2(this.canvas.width / (this.canvas.width), this.canvas.height / (1.5 * this.SCALE)),
    ...
    updatePlayer() {
        ...
        if (Math.abs(playerPosition.y - (this.canvas.height / this.SCALE)) < 1.1) {
            // move player to start
            this.player.setPosition(planck.Vec2(
                this.canvas.width / (this.canvas.width), this.canvas.height / (1.5 * this.SCALE)
            ));
            this.player.setLinearVelocity(planck.Vec2(0, 0));
        }

        // check if player wins
        if (Math.abs(playerPosition.x - (this.canvas.width / this.SCALE)) < 1.1) {
            this.loadNextLevel();
        }
    }
    ...
    loadNextLevel() {
        $.ajax({
            ...
            success: (levelIds) => {
                const $levelSelect = $("#level-select");
                const currentLevel = $levelSelect.val();
                const currentLevelIndex = levelIds.indexOf(currentLevel);

                // Check if there is a next level
                if (currentLevelIndex + 2 < levelIds.length) {
                    const nextLevelId = levelIds[currentLevelIndex + 1];

                    // Update the level select dropdown to reflect the next level
                    $levelSelect.val(nextLevelId);

                    // Load the next level data
                    $.ajax({
                        url: `http://localhost:3000/level/${encodeURIComponent(nextLevelId)}`,
                        method: "GET",
                        success: (levelData) => {
                            this.createLevel(levelData);  // Load the next level
                        },
                        error: (xhr, status, error) => {
